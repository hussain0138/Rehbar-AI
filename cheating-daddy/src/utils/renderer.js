...
async function initializeGemini(profile = 'interview', language = 'en-US') {
    ...
    if (apiKey) {
        ...
        if (success) {
            rehbar.setStatus('Live');
        } else {
            rehbar.setStatus('error');
        }
    }
}
...
ipcRenderer.on('update-status', (event, status) => {
    ...
    rehbar.setStatus(status);
});

// Listen for responses - REMOVED: This is handled in RehbarAiApp.js to avoid duplicates
// ipcRenderer.on('update-response', (event, response) => {
//     console.log('Gemini response:', response);
//     rehbar.e().setResponse(response);
...
function handleShortcut(shortcutKey) {
    const currentView = rehbar.getCurrentView();

    if (shortcutKey === 'ctrl+enter' || shortcutKey === 'cmd+enter') {
        if (currentView === 'main') {
            rehbar.element().handleStart();
        } else {
...
// Consolidated rehbar object - all functions in one place
const rehbar = {
    // Element access - dynamic to ensure element is available
    element: () => document.querySelector('rehbar-ai-app'),
    e: () => document.querySelector('rehbar-ai-app'),

    // App state functions - access properties directly from the app element
    getCurrentView: () => document.querySelector('rehbar-ai-app')?.currentView,
    getLayoutMode: () => document.querySelector('rehbar-ai-app')?.layoutMode,

    // Status and response functions
    setStatus: text => document.querySelector('rehbar-ai-app')?.setStatus(text),
    setResponse: response => document.querySelector('rehbar-ai-app')?.setResponse(response),
...
window.rehbar = rehbar;

// Ensure the app element is ready
document.addEventListener('DOMContentLoaded', () => {
    // Wait a bit more for custom elements to be defined
    setTimeout(() => {
        const appElement = document.querySelector('rehbar-ai-app');
        if (appElement) {
            console.log('Rehbar AI app element found and ready');
        } else {
            console.warn('Rehbar AI app element not found');
        }
    }, 100);
});
