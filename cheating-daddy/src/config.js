...
function getConfigDir() {
    ...
    if (platform === 'win32') {
        // Windows: %APPDATA%\rehbar-ai-config
        configDir = path.join(os.homedir(), 'AppData', 'Roaming', 'rehbar-ai-config');
    } else if (platform === 'darwin') {
        // macOS: ~/Library/Application Support/rehbar-ai-config
        configDir = path.join(os.homedir(), 'Library', 'Application Support', 'rehbar-ai-config');
    } else {
        // Linux and others: ~/.config/rehbar-ai-config
        configDir = path.join(os.homedir(), '.config', 'rehbar-ai-config');
    }
